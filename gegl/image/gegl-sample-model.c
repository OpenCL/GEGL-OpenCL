...
static void set_pixel_normalized(const GeglSampleModel* self,
                          ...
                          GeglBuffer* buffer);
static GeglNormalizer* get_normalizer(const GeglSampleModel* self, gint band);
GType
                              ...
                              const GeglBuffer* buffer)
{
    ...
    gdouble sample=gegl_sample_model_get_sample_double(self,x,y,band,buffer);
                              ...
                              const GeglBuffer* buffer)
{
    ...
    for (i=0;i<self->num_bands;i++) {
        ...
        if (normalizer !=NULL) {
            gegl_normalizer_normalize(normalizer,nor_data,nor_data,1,self->num_bands);
        }
    }
    return nor_data;
}
                          ...
                          GeglBuffer* buffer)
{
    ...
    if (self->normalizers!=NULL) {
        for (i=0;i<self->num_bands;i++) {
            GeglNormalizer* normalizer=get_normalizer(self,i);
            gegl_normalizer_unnormalize(normalizer,nor_data,unnor_data,1,self->num_bands);
        }
