...
static void a_iadd_b_float (GeglFilter * filter, GeglImageBufferIterator ** iters, gint width);
static void a_iadd_b_uint8 (GeglFilter * filter, GeglImageBufferIterator ** iters, gint width);                       
...
{                                                                       
  ...
  guint8 **b = (guint8**)gegl_image_buffer_iterator_color_channels(iters[1]);
  guint8 *ba = (guint8*)gegl_image_buffer_iterator_alpha_channel(iters[1]);
  gint b_color_chans = gegl_image_buffer_iterator_get_num_colors(iters[1]);

  guint8 **a = (guint8**)gegl_image_buffer_iterator_color_channels(iters[2]);
  guint8 *aa = (guint8*)gegl_image_buffer_iterator_alpha_channel(iters[2]);
  gint a_color_chans = gegl_image_buffer_iterator_get_num_colors(iters[2]);
...

  {
    ...
    while(width--)                                                        
      {                                                                   
        switch(d_color_chans)
          {
            case 3: *d2++ = CLAMP((gint)(fade * *a2 + *b2 + .5), 0, 255); 
                    a2++; 
                    b2++;
            case 2: *d1++ = CLAMP((gint)(fade * *a1 + *b1 + .5), 0, 255);
                    a1++; 
                    b1++;
        ...
        if(alpha_mask == GEGL_A_B_ALPHA)
          {
              *da++ = CLAMP((gint)(fade * *aa + *ba + .5), 0, 255);
               aa++;
               ba++;
          }
      }
