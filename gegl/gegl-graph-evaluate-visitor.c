...
static void class_init (GeglGraphEvaluateVisitorClass * klass);
static void init (GeglGraphEvaluateVisitor * self, GeglGraphEvaluateVisitorClass * klass);
...
static void validate_inputs(GeglGraphEvaluateVisitor *self, GeglFilter * filter);
static void validate_outputs(GeglGraphEvaluateVisitor *self, GeglFilter * filter);

static void validate_op_outputs(GeglGraphEvaluateVisitor *self, GeglFilter * filter);
static void validate_image_outputs(GeglGraphEvaluateVisitor *self, GeglFilter * filter);

static gpointer parent_class = NULL;

GType
gegl_graph_evaluate_visitor_get_type (void)
{
  static GType type = 0;
  ...
  if (!type)
    {
      ...
      {
        sizeof (GeglGraphEvaluateVisitorClass),
        (GBaseInitFunc) NULL,
        (GBaseFinalizeFunc) NULL,
        (GClassInitFunc) class_init,
        (GClassFinalizeFunc) NULL,
        NULL,
        sizeof (GeglGraphEvaluateVisitor),
        0,
        (GInstanceInitFunc) init,
      };

      type = g_type_register_static (GEGL_TYPE_DFS_VISITOR, 
                                     "GeglGraphEvaluateVisitor", 
                                     &typeInfo, 
                                     0);
    }
    return type;
...
static void 
class_init (GeglGraphEvaluateVisitorClass * klass)
{
...
static void 
init (GeglGraphEvaluateVisitor * self, 
      GeglGraphEvaluateVisitorClass * klass)
{
  self->graph = NULL;
}
...
visit_filter(GeglVisitor * visitor,
             GeglFilter *filter)
{
  GeglGraphEvaluateVisitor *self = GEGL_GRAPH_EVALUATE_VISITOR(visitor); 
  ...
  input_attributes = gegl_graph_get_input_attributes(self->graph, GEGL_NODE(filter));
  ...
  gegl_dump_graph(GEGL_NODE(self->graph));
...
void
validate_inputs(GeglGraphEvaluateVisitor *self,
                GeglFilter * filter)
{
  gint i;
  gint num_inputs = gegl_node_get_num_inputs(GEGL_NODE(filter)); 
  GList *inputs_attributes_list = 
    gegl_graph_get_input_attributes(self->graph, 
                                    GEGL_NODE(filter));
...
static void 
validate_outputs(GeglGraphEvaluateVisitor *self,
                 GeglFilter * filter) 
{
...
static void
validate_op_outputs(GeglGraphEvaluateVisitor *self,
                    GeglFilter * filter) 
{
...
static void
validate_image_outputs(GeglGraphEvaluateVisitor *self,
                       GeglFilter * filter) 
{
      ...
      if(!tile) 
        {
          ...
          g_value_set_object(attributes->value, image_tile);
        }
      else 
        {
          ...
          /* Put in output value. */
          g_value_set_object(attributes->value, tile);
        }
