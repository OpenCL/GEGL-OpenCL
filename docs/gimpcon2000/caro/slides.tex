...
\nfoil{Assumptions we make}
\begin{itemize}
\item Channel data are pointers 
\item Variables are assigned value outside GIL  
\end{itemize}
...
\item Sets Pixel variable equal to data value 
\end{itemize}
...
\Large
\item a = EXTERNAL\_VARIBLE (v);\\
\small {\hspace*{.4cm} *a\_red = v[0].u16\_k;\\*[-.5cm]
\hspace*{.4cm} *a\_green = v[1].u16\_k;\\*[-.5cm]
\hspace*{.4cm} *a\_blue = v[2].u16\_k;\\*[-.5cm]
\hspace*{.4cm} if (d\_has\_alpha)\\*[-.5cm]
\hspace*{.8cm} *a\_alpha = v[3].u16\_k;} 
\Large
\item a = a / 5.0;\\
...
\small {\hspace*{.4cm} *a\_alpha = (*a\_alpha * *a\_alpha) / 255L + ((255L - *a\_alpha) * *a\_alpha) / 255L;}
\Large 
\end{itemize}
...
\nfoil{Limitations}
\begin{itemize}
\item Channel of different ranges 
\end{itemize}
...
\begin{itemize}
\small
\item http://siggrahp.gimp.org 
\end{itemize}
\end{itemize}

\nfoil{Thanks}
Many thanks to Calvin Williamson, The Gimp community, Rhythm \& Hues, and a special thanks to our sponcers the Free Software Foundation, the Chaos Computer Club, and O'Reilly Germany 
