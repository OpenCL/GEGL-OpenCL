...
{
  {
    ...
    ct_test(test, GEGL_IS_MOCK_DFS_VISITOR(mock_dfs_visitor));
    ct_test(test, g_type_parent(GEGL_TYPE_MOCK_DFS_VISITOR) == GEGL_TYPE_DFS_VISITOR);
    ct_test(test, !strcmp("GeglMockDfsVisitor", g_type_name(GEGL_TYPE_MOCK_DFS_VISITOR)));
...
**/
static void
test_dfs_visitor_traverse_graph(Test *test)
{
  {
    gint i;
...
static void
test_dfs_visitor_simple_shared(Test *test)
{
...
{
  Test *t = ct_create("GeglDfsVisitorTest");
