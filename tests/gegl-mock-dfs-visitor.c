...
static void class_init (GeglMockDfsVisitorClass * klass);
...
GType
gegl_mock_dfs_visitor_get_type (void)
{
  static GType type = 0;
  ...
  if (!type)
    {
      ...
      {
        sizeof (GeglMockDfsVisitorClass),
        (GBaseInitFunc) NULL,
        (GBaseFinalizeFunc) NULL,
        (GClassInitFunc) class_init,
        (GClassFinalizeFunc) NULL,
        NULL,
        sizeof (GeglMockDfsVisitor),
        0,
...
static void 
class_init (GeglMockDfsVisitorClass * klass)
{
...
visit_graph(GeglVisitor * visitor,
             GeglGraph *graph)
{
  gegl_dfs_visitor_traverse(GEGL_DFS_VISITOR(visitor), graph->root);
